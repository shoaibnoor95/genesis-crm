                                        ...
                                        {filter.map((v, i) => {
                                            ...
                                            return (
                                                <tr key={i}>
                                                    <td>{v._id}</td>
                                                    <td>{v.FullName}</td>
                                                    <td>{v.status.statusAdmin === "Transfer" || v.status.statusAdmin === "Dublicated" ? <b>✦✦✦✦✦✦✦✦✦✦</b> : v.phone}</td>
                                                    <td>{v.Time}</td>
                                                    <td>{v.createdAt}</td>
                                                    ...
                                                    <td>
                                                        {v.status.statusAdmin === "Dublicated" ?
                                                            ...
                                                            :
                                                            <Button size="sm" color={v.status.statusAdmin === "Transfer" ? 'outline-dark' : 'outline-primary'} ><i className={v.status.statusAdmin === "Transfer" ? "fa fa-exclamation-circle" : "fa fa-search"}></i> {v.status.statusAdmin === "Transfer" ? "Disabled" : "View"}</Button>
                                                        }
                                                    </td>
                                                    <td>
                                                        <Button title={`To ${v.status.transferCloserName}`} size="sm" color={'outline-danger'} onClick={this.kickBack.bind(this, transferAgentID, transferAgentName, v.key, transferCloserID, transferCloserName, transferDate, v.date, i, transferAdminID, transferAdminName)}><i className="fa fa-backward"></i> Kick Back</Button>
                                                    </td>
                                                    <td>
                                                        ...
                                                        <div id="transferModal" className="modal" style={{ position: 'fixed', zIndex: 1, paddingTop: '100px', left: 0, top: 0, right: 0, width: '40%', height: '100%', overflow: 'auto', margin: "0 auto", marginTop: "90px", display: this.state.transferModal[i].display }}>
                                                            <div className="modal-content">
                                                                ...
                                                                <Row>
                                                                    <Col>
                                                                        <Label htmlFor="vat">Agent: {v.status.transferAgentName}</Label>
                                                                        <select
                                                                            onChange={(e) => {
                                                                                let { statusAgent } = this.state;
                                                                                this.setState({ statusAgent: e.target.value })
                                                                            }}
                                                                            className="format" value={this.state.statusAgent === "" ? v.status.status : this.state.statusAgent}>
                                                                ...
                                                                <Row>
                                                                    <Col>
                                                                        <Label htmlFor="vat">Closer: {v.status.transferCloserName}</Label>
                                                                        <select
                                                                            onChange={(e) => {
                                                                                let { statusCloser } = this.state;
                                                                                this.setState({ statusCloser: e.target.value })
                                                                            }}
                                                                            className="format" value={this.state.statusCloser === "" ? v.status.statusCloser : this.state.statusCloser}>
                                                                            {/* <option value="Transfer">Transfer</option>
                                                                            <option value="Pending">Pending</option>
                                                                            <option value="Approved">Approved</option>
                                                                            <option value="Rejected">Rejected</option>
                                                                ...
                                                                <Button size="sm" color={'success'} onClick={this.updateStatus.bind(this, v.status.transferAgentID, v.status.transferCloserID, v.status.transferAdminID, v.key)} style={{ padding: 6, marginTop: 20 }}><i className="fa fa-exchange"></i>{" "}Update Status</Button>
                                                    ...
                                                    <td>
                                                        <Workbook filename={v.ID + ".xlsx"} element={<div className="clearfix">
