...
class TodaySell extends Component {
    ...
    //     await firebase.database().ref('/').child(`NewDeals/${date}/${uid}`).on('value', (data) => {
    //         this.setState({ data: [] })
    //         setTimeout(() => {
    //             for (var key in data.val()) {
    //                 this.state.transferModal.push({ display: "none" })
    //                 this.state.data.push(data.val()[key])
    //                 this.setState({ data: this.state.data, transferModal: this.state.transferModal,pointerEvents: "auto" })
    //             }
    //         }, 10)
    //     })
...
export default connect(mapStateToProp, mapDispatchToProp)(TodaySell);
