...
class TodaySell extends Component {
    ...
    //         this.setState({ data: [] })
    //         console.log("sale=====>", data.val())
    //         setTimeout(() => {
    //             for (var key in data.val()) {
    //                 this.state.transferModal.push({ display: "none" })
    //                 this.state.data.push(data.val()[key])
    //                 this.setState({ data: this.state.data, transferModal: this.state.transferModal })
    //             }
    //         }, 10)
    //     })
    ...
    //     // console.log("=>>>>>>>", user)
    //     let closer = "Closer"
    //     this.props.getUser(user, closer)
                                        ...
                                        {filter.map((v, i) => {
                                            ...
                                            return (
                                                <tr key={i}>
                                                    <td>{v.ID}</td>
                                                    <td>{v.fullName}</td>
                                                    <td>{v.status.statusCloser === "Transfer" || v.status.statusCloser === "Dublicated" ? <b>✦✦✦✦✦✦✦✦✦✦</b> : v.phone}</td>
                                                    <td>{v.time}</td>
                                                    <td>{v.date}</td>
                                                    <td title={v.status.statusCloser === "Call Back" ? v.status.callbackDate + " " + v.status.callbackTime : ""}>
                                                        <Badge style={{ width: 100, height: 25, paddingTop: 7, backgroundColor: v.status.statusCloser === "Pending" ? '#ffab00' : v.status.statusCloser === "Transfer" ? '#e4e5e6' : v.status.statusCloser === "Kick Back" ? "#ff7043" : v.status.statusCloser === "Dublicated" ? "#ff8a65" : v.status.statusCloser === "Approved" ? '#4caf50' : v.status.statusCloser === "Rejected" ? "#d32f2f" : '#81d4fa' }}><i className={v.status.statusCloser === "Pending" ? "fa fa-spinner" : v.status.statusCloser === "Transfer" ? "fa fa-exchange" : v.status.statusCloser === "Kick Back" ? "fa fa-backward" : v.status.statusCloser === "Dublicated" ? "fa fa-clone" : v.status.statusCloser === "Approved" ? "fa fa-check" : v.status.statusCloser === "Rejected" ? "fa fa-exclamation-triangle" : "fa fa-phone"}></i>{v.status.statusCloser === "Call Back" ? <i className="fa fa-clock"></i> : " "} {v.status.statusCloser}</Badge>
                                                    </td>
                                                    <td>
                                                        {v.status.statusCloser === "Dublicated" ?
                                                            <Button size="sm" color={'outline-dark'} disabled><i className="fa fa-exclamation-circle"></i> Disabled</Button>
                                                            :
                                                            <Button size="sm" color={v.status.statusCloser === "Transfer" ? 'outline-dark' : 'outline-primary'} onClick={this.showModal.bind(this, i)} disabled={v.status.statusCloser === "Transfer" ? true : v.status.statusCloser === "Dublicated" ? true : false}><i className={v.status.statusCloser === "Transfer" ? "fa fa-exclamation-circle" : "fa fa-search"}></i> {v.status.statusCloser === "Transfer" ? "Disabled" : "View"}</Button>
                                                        }
                                                    </td>
                                                    <td>
                                                        {v.uid === user.uid || v.status.statusCloser === "Transfer" ?
                                                            <Button title={"Own"} size="sm" color={'outline-dark'} disabled><i className="fa fa-exclamation-circle"></i> Disabled</Button>
                                                            :
                                                            <Button title={`To ${v.status.transferAgentName}`} size="sm" color={'outline-danger'} onClick={this.kickBack.bind(this, transferAgentID, transferAgentName, v.key, transferCloserID, transferCloserName, transferDate, v.date, i)}><i className="fa fa-backward"></i> Kick Back</Button>
                                                        }
                                                    </td>                                                    {v.status.statusCloser === "Transfer" ?
                                                        <td style={{ textDecoration: "underline" }}><Button title={"To " + v.status.transferAdminName} size="sm" color={'outline-dark'} disabled><i className="fa fa-check"></i> Transferred</Button></td>
                                                        :
                                                        <td>
